...
int create_nn(NN* nn) {
    ...
    for(int i = 0; i < nn->num_layers; i++) {
        ...
        if(layer->layer_type == LAYER_TYPE_MLP || layer->layer_type == LAYER_TYPE_LAYERNORM || layer->layer_type == LAYER_TYPE_SOFTMAX || layer->layer_type == LAYER_TYPE_WEIGHTSTENSORMULTIPLICATION) {
